...
ID3D12DescriptorHeap *DirectXCommon::CreateDescriptorHeap(
	...
	const HRESULT hr = device_->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&heap));
	assert(SUCCEEDED(hr));
	return heap;
}

D3D12_CPU_DESCRIPTOR_HANDLE DirectXCommon::GetCPUHandle(
	ID3D12DescriptorHeap *heap, UINT index) const noexcept {
...
D3D12_GPU_DESCRIPTOR_HANDLE DirectXCommon::GetGPUHandle(
	ID3D12DescriptorHeap *heap, UINT index) const noexcept {
...
void DirectXCommon::WaitForFrame(UINT frameIndex) {
	...
	if (fenceValue == 0) return;                               // まだ Signal していない
	if (fence_->GetCompletedValue() >= fenceValue) return;     // 既に完了
...
void DirectXCommon::Initialize(WinApp *winApp) {
	...
	width_ = static_cast<uint32_t>(std::max<LONG>(1, rc.right - rc.left));
	height_ = static_cast<uint32_t>(std::max<LONG>(1, rc.bottom - rc.top));
...
void DirectXCommon::PreDraw(const float clearColor[4]) noexcept {
	// 最小化中は何もしない（WM_SIZE ハンドラ側で 0x0 になる場合の保険）
	...
	const D3D12_CPU_DESCRIPTOR_HANDLE rtv = rtvHandles_[currentBackBufferIndex_];
...
void DirectXCommon::PostDraw() noexcept {
	if (width_ == 0 || height_ == 0) return;
	...
	const HRESULT  hrSignal = commandQueue_->Signal(fence_.Get(), fenceToSignal);
...
void DirectXCommon::Resize(uint32_t width, uint32_t height) {
	...
	// 古いバックバッファ類を解放
	for (UINT i = 0; i < kBufferCount; ++i) backBuffers_[i].Reset();
	depthStencil_.Reset();
	...
	const HRESULT hr = swapChain_->ResizeBuffers(
		...
		0 /* tearing を使うなら DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING */);
	assert(SUCCEEDED(hr));
...
void DirectXCommon::InitializeFixFPS() noexcept {
...
void DirectXCommon::InitializeDevice() {
	...
	for (UINT i = 0;
		...
		if ((desc.Flags & DXGI_ADAPTER_FLAG3_SOFTWARE) == 0) break;
		adapter_.Reset();
	}
	assert(adapter_);
...
void DirectXCommon::InitializeSwapChain() {
	...
	const HRESULT hr = dxgiFactory_->CreateSwapChainForHwnd(
		commandQueue_.Get(), winApp_->GetHwnd(), &desc, nullptr, nullptr, sc1.GetAddressOf());
	assert(SUCCEEDED(hr));

	// IDXGISwapChain4 に昇格
	const HRESULT hr2 = sc1.As(&swapChain_);
	assert(SUCCEEDED(hr2));

	// Alt+Enter のフルスクリーン切替を無効化（アプリ側で制御）
...
void DirectXCommon::InitializeDescriptorHeaps() {
	...
	// 必要なヒープ作成
	rtvHeap_.Attach(CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_RTV, kBufferCount, false));
	dsvHeap_.Attach(CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1, false));
	srvHeap_.Attach(CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 128, true));
}
...
void DirectXCommon::InitializeDepthStencilView() {
	...
	device_->CreateDepthStencilView(depthStencil_.Get(), &desc, dsvHeap_->GetCPUDescriptorHandleForHeapStart());
}
...
void DirectXCommon::InitializeFence() {
	...
	nextFenceValue_ = 0;
	for (UINT i = 0; i < kBufferCount; ++i) fenceValues_[i] = 0;
}

void DirectXCommon::InitializeViewport() noexcept {
...
void DirectXCommon::InitializeScissorRect() noexcept {
	scissorRect_.left = 0;
	scissorRect_.top = 0;
...
void DirectXCommon::UpdateFixFPS() noexcept {
	...
	if (elapsed < kMinTime) {
		// できるだけ OS に譲ってから、残りを busy-wait せず 1ms 粒度で眠る
