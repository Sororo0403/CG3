...
void DirectXCommon::InitializeDescriptorHeaps_() {
	...
	rtvHeap_.Attach(CreateDescriptorHeap_(
		D3D12_DESCRIPTOR_HEAP_TYPE_RTV, kBufferCount, false));
	dsvHeap_.Attach(CreateDescriptorHeap_(
		D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1, false));
	srvHeap_.Attach(CreateDescriptorHeap_(
...
void DirectXCommon::InitializeDepthBuffer_() {
	// Tex2D のヘルパーと HEAP_PROPERTIES, CLEAR_VALUE を使用
...
void DirectXCommon::InitializeRenderTargetViews_() {
	...
	for (UINT i = 0; i < kBufferCount; ++i) {
		rtvHandles_[i] = base;
		rtvHandles_[i].ptr +=
			static_cast<SIZE_T>(i) * descriptorSizeRTV_;
		device_->CreateRenderTargetView(
			backBuffers_[i].Get(), &rtvDesc, rtvHandles_[i]);
	}
}
...
void DirectXCommon::InitializeViewport_() {
	...
	viewport_.Width =
		static_cast<float>(winApp_->GetWidth());
	viewport_.Height =
		static_cast<float>(winApp_->GetHeight());
