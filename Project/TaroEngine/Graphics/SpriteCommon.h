...
public:
  ...
  struct PipelineFormats {
    DXGI_FORMAT rtvFormat =
        DXGI_FORMAT_R8G8B8A8_UNORM; ///< レンダーターゲットのフォーマット
    DXGI_FORMAT dsvFormat =
        DXGI_FORMAT_UNKNOWN;   ///< 深度ステンシルのフォーマット（未使用なら
                               ///< UNKNOWN）
...
public:
  ...
  void CreateGraphicsPipeline(IDxcUtils *dxcUtils, IDxcCompiler3 *dxcCompiler,
  ...
  /// <param name="topology">プリミティブトポロジ（デフォルト:
  /// 三角形リスト）</param>
  void ApplyCommonDrawSettings(ID3D12GraphicsCommandList *cmd,
...
private:
  ID3D12Device *device_; ///< D3D12 デバイス

  Microsoft::WRL::ComPtr<ID3D12RootSignature>
      rootSignature_; ///< ルートシグネチャ
  Microsoft::WRL::ComPtr<ID3D12PipelineState>
      pipelineState_; ///< パイプラインステート
};
